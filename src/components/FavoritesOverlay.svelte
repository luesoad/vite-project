<script lang="ts">
  ...
  export let iconOnly = false; // für Mobilanzeige

  let open = false;

  const allConcerts: (Concert & { year?: number })[] = concertsData.flatMap(el =>
...
<!-- Button: immer rendern, aber bei 0 Favoriten deaktiviert & ausgegraut -->
<button
  class="favorites-btn flex items-center gap-2 px-4 py-2 transition-opacity duration-200
         md:px-3 md:py-1
         text-base md:text-base
         {($favorites.length === 0 ? 'opacity-50 pointer-events-none' : 'cursor-pointer')}"
  aria-label="Show favorites"
  on:click={() => $favorites.length > 0 && (open = !open)}
  style="background: none; border: none; outline: none;"
>
  <span class="text-lg md:text-base"
        style="color: var(--color-accent, #eab308); font-size: 1.2em;">★</span>
  {#if !iconOnly}
        ...
        {:else}
          <ul class="mb-4 space-y-4">
            {#each getFavoriteConcerts() as c}
              <li class="text-black break-words flex items-center justify-between gap-2">
                <div>
                  <span class="font-semibold">{c.date} {c.time ? c.time : ''}</span> – 
